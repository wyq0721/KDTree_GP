...
static void resize(struct kdtree *tree)
{
        ...
        tree->coord_deleted = realloc(tree->coord_deleted, sizeof(unsigned char) * tree->capacity);
        tree->coord_passed = realloc(tree->coord_passed, sizeof(unsigned char) * tree->capacity);
...
static void knn_pickup(struct kdtree *tree, struct kdnode *node, double *target, int k)
{
        ...
        if (tree->knn_num < k) {
                ...
                } else if (fabs(dist - knn_max(tree)) < DBL_EPSILON) { // Double_Epsilon双精度最小误差
                        knn_list_add(tree, node, dist);
                }
        }
}
